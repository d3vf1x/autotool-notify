...
def main():
  ...
  if os.path.isfile(dir + "stopp.txt"):
    print_log("stopp.txt wurde gefunden, beende",0)
    sys.exit(0)
  ...
  try:
      ...
      if os.stat(dir + "cookies.txt").st_size != 0:
        ...
        if aktuelleAufgaben.status_code == 200:
          ...
          if(html.count('Pflichtaufgaben haben Sie bis jetzt') > 0):
            ...
            print_log("Session abgelaufen, erneuter Login erforderlich",0)
        else:
          raise Exception(str(aktuelleAufgaben.status_code) + "Beim 1. versuch aufgaben aktuell zu laden")
      else:
        print_log("keine gespeicherte Session gefunden", 0)
    
      # erneuter Login
      with requests.Session() as s:
        print_log("starte loginprozedur",0)
        ...
        postresponse = s.post('https://autotool.imn.htwk-leipzig.de/Shibboleth.sso/SAML2/POST',data=payload, cookies={})
        if postresponse.status_code != 200:
    ...
    f.close()
    send_Message('Es ist ein Fehler aufgetreten!\n Crontab on halt!\n' + repr(error))
